...
hotels %>% 
  # however you want!!!
  select(hotel, lead_time) %>% # Here i select only the two variables
  arrange(desc(lead_time)) # you can remove desc to order in ascending way

# Here is a comment
hotels %>%
  filter(arrival_date_month %in% c("June", "July"))
...
hotels %>%
  ...
  group_by(hotel) %>%
  summarise(mean_child = mean(little_ones, na.rm = T))
